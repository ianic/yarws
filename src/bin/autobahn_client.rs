...
async fn main() -> Result<(), Error> {
    ...
    generate_report(&args.url).await?;
...
async fn generate_report(root: &str) -> Result<(), Error> {
...
async fn get_case_count(root: &str) -> Result<usize, Error> {
    let url = root.to_owned() + "/getCaseCount";
    let mut socket = connect(&url).await?.into_text();
